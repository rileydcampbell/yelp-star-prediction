...
class YelpClassifier(object):
    ...
    def calculate_likelihoods(self, train_rdd):
        ...
        class_likelihoods = train_rdd                             .flatMap(self.review_to_word_counts)                             .reduceByKey(self.add_review_counts)                             .map(self.counts_to_probabilities)                             .aggregateByKey({}, self.combine_probability_tables, self.combine_probability_tables)
    ...
    def classify_reviews(self, test_rdd):
        ...
        predictions = test_rdd                             .flatMap(self.review_to_num_stars_and_word_pairs)                             .map(self.words_to_log_likelihoods)                             .reduceByKey(self.add_log_likelihoods)                             .map(self.likelihood_to_posterior)                             .map(self.review_id_only_as_key)                             .reduceByKey(self.find_max_posterior)                             .sortByKey()
